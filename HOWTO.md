...
- **受け取った `.vsix` の二次配布は禁止です**（研究室外の人に渡す・どこかにアップロードする等）。この vsix は Marketplace の審査や署名を経ていない手渡し配布なので、「配布者から直接受け取った」という事実だけが本物である保証です。人づてに転送が重なると、途中で改ざんされたファイルが混ざっても受け取った人には見分けられません。使いたい人がいたら配布者（宮内）に直接連絡してください。

## 1. インストール

配布された `ai-code-guide.vsix` を受け取ったら、ターミナルで:
